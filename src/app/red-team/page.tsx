...
export default function RedTeamPage() {
  ...
  const fullText = "Projets Red Team";
  const [projects, setProjects] = useState<Project[]>([]);

  // Charger les projets Red Team
  useEffect(() => {
    setProjects(getProjectsByCategory("Red Team"));
  }, []);
  ...
  // Force le thème rouge pour cette page
  useEffect(() => {
    setTheme("red");
    // Restauration impossible car cette page doit toujours être rouge
    return () => {};
  }, [setTheme]);

  return (
    <main className="min-h-screen bg-gradient-to-b from-red-950 via-black to-red-950 text-white">
      <Header />
      <div className="pt-32 pb-20 gemini-container">
        <div className="mb-12 text-center">
          ...
          >
            Collection de mes projets offensifs, outils et recherches dans le domaine de la sécurité offensive.
          </motion.p>
        </div>
            ...
            >
              ...
              <div className="flex flex-wrap gap-2 mb-4">
                {project.techStack.map((tech) => (
                  <span
                    key={tech}
                    className="px-2 py-1 bg-slate-800 rounded-full text-xs hover:bg-red-900 transition-colors"
                  >
                    {tech}
                  </span>
                ))}
